 ...
 */
fun subtractOf(a: MutableMap<String, String>, b: Map<String, String>) = b.forEach { (k, v) -> a.remove(k, v) }
...
fun averageStockPrice(stockPrices: List<Pair<String, Double>>): Map<String, Double> =
    stockPrices.groupBy({ it.first }, { it.second })
...
fun findCheapestStuff(stuff: Map<String, Pair<String, Double>>, kind: String): String? =
    stuff.filter { it.value.first == kind }
        .minByOrNull { it.value.second }?.key
...
fun findSumOfTwo(list: List<Int>, number: Int): Pair<Int, Int> {
    ...
    for (i in list.indices) {
        for (j in list.indices) {
            if (i == j) continue
            result = if (list[i] + list[j] == number) {
