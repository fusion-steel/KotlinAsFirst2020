...
fun diameter(vararg points: Point): Segment {
    ...
    if (points.size >= 2) {
        ...
        throw IllegalArgumentException("Для расчета необходимо минимум 2 точки.")
    }
    return segments.maxByOrNull { it.begin.distance(it.end) }!!
}
...
class Line private constructor(val b: Double, val angle: Double) {
    ...
    fun crossPoint(other: Line): Point {
        val x = (cos(angle) * other.b - cos(other.angle) * b) /
                (sin(angle) * cos(other.angle) - cos(angle) * sin(other.angle))
...
fun bisectorByPoints(a: Point, b: Point): Line {
    val centerPoint = Point((a.x + b.x) / 2, (a.y + b.y) / 2)
    val angle = atan2(b.y - a.y, b.x - a.x)
    return Line(centerPoint, (angle + PI / 2) % PI)
}
